...
7. 同一角色的 automation 不得并发推进；任何 run 在开始实质工作前都必须先拿到该角色的锁。
...
- 开始实质工作前先获取 `worker` 角色锁，并在 run 退出前释放
- 从隔离 worktree 启动
- 编辑前先创建或切换到命名分支
- 一次只推进一个连贯子任务
...
- 开始实质工作前先获取 `curator` 角色锁，并在 run 退出前释放
- 审查候选 worker 分支
- 选一个整合基线
...
- `worker` 和 `curator` 角色共用 `/tmp/grounded-deck-automation-locks/` 下的全局角色锁；拿不到锁的 run 必须立即停止。
- 分支名应当能表达归属和主题。
- 优先使用这些前缀：
...
1. 工作位于命名分支上
2. 本次 run 已成功获取并持有对应角色锁
3. `make eval` 通过
4. canonical state docs 已反映最新事实
5. worker 输出之间没有遗留未解决冲突
6. 如果声称完成了 live verification，则 `reports/` 中必须存在归档后的验证工件
...
- 仓库现在支持带角色锁的多车道 automation：3 条 worker 时隙（`:00`、`:10`、`:20`）和 3 条 curator 时隙（`:30`、`:40`、`:50`）
